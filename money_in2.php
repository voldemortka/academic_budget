<?php
    ...
    if(isset($_POST['x']) && $_COOKIE['lim_OK']){
        ...
        while($row = pg_fetch_assoc($result)){
            echo $_POST['l'.$name];
            $limit=0;
            if($_POST[$row['name']]=='save') $sav += $row['curr_money'];
            else $limit = $row['curr_money'];
            $limit += $_POST['l'.$row['name']];
            $sql2 = "update categories set lim=".$limit." curr_money=".$limit." where name='".$row['name']."' and acc=".$id;
            pg_query($conn, $sql);
        }

        $sql = "update account set savings=".$sav;
        pg_query($conn, $sql);
...
<html lang='en'>
    ...
    <body>
        ...
        <main>
        <form action='money_in2.php' method='post'>
            <input type='radio' name='x' checked>
            <h2>Here there're money that you still have from the previous mounth:</h2>
            <p>Choose what to do with them</p>
            <p>And set new limits for the new mouth</p>
